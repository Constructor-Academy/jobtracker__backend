...
def code_is_valid(code, email):
    try:
        reg_profile = RegistrationProfile.objects.get(user__email=email)
        if reg_profile.code_used:
            raise ValidationError(message='This code has already been used!')
        if reg_profile.code != code:
...
class PasswordResetValidationSerializer(serializers.Serializer):
    ...
    def save(self, validated_data):
        email = validated_data.get('email')
        user = RegistrationProfile.objects.get(user__email=email).user
        user.set_password(validated_data.get('password'))
        user.registration_profile.code_used = True
        user.save()
        user.registration_profile.save()
